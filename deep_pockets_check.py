...
def detect_deep_pockets(mesh, tool_directions=None, threshold=25.0):
    if tool_directions is None:
        tool_directions = [np.array([1,0,0]), np.array([0,1,0]), np.array([0,0,1])]
    max_depth = 0
    best_dir = None
    for direction in tool_directions:
    ...
    return {
        'max_depth': max_depth,
        'tool_direction': best_dir.tolist() if best_dir is not None else None,
        'has_problem': has_problem,
        'severity': 2 if max_depth > 2*threshold else 1 if has_problem else 0,
        'recommendation': f"Deep pocket detected ({max_depth:.1f}mm). Consider reducing depth or using longer tools." if has_problem else "No deep pockets detected."
    }